  ...
  <div class="app-container">
    <el-form :inline="true" class="demo-form-inline">
      <el-form-item label="订单号">
        <el-input v-model="listQuery.orderSn" placeholder="请输入订单号"></el-input>
      </el-form-item>
      <el-form-item label="编号">
        <el-input v-model="listQuery.sortingNo" placeholder="请输入编号"></el-input>
      </el-form-item>
      <el-form-item label="手机号">
        <el-input v-model="listQuery.mobile" placeholder="请输入手机号"></el-input>
      </el-form-item>
      <el-form-item label="发货日期">
        <el-date-picker v-model="listQuery.date" type="date" value-format="yyyy-MM-dd" placeholder="选择日期"></el-date-picker>
      </el-form-item>
    ...
    >
      ...
      <el-table-column align="center" label="操作" min-width="150">
        <template slot-scope="scope">
          <el-button size="small" @click="detailOrder(scope)">查询商品</el-button>
          <el-button
            type="primary"
            size="small"
            @click="express(scope)"
      ...
      <div>
        <el-table :data="detail.waybillTracker" style="margin-top:20px;" border>
          <el-table-column label="详细">
            <template
              slot-scope="scope"
            >{{scope.row.time}}&nbsp;&nbsp;&nbsp;&nbsp;{{ scope.row.info }}</template>
          </el-table-column>
        </el-table>
      </div>
      <div style="text-align:right;">
        <el-button type="danger" @click="dialogVisibleDetail=false">取消</el-button>
      </div>
    </el-dialog>
    <el-dialog :visible.sync="dialogVisibleGoods" title="商品">
      <div>
        <el-table :data="showGoods" style="margin-top:20px;" border>
          ...
          <el-table-column align="center" label="规格" width="160">
            <template slot-scope="scope">{{ scope.row.specifications[0] }}</template>
          </el-table-column>
      ...
      <div style="text-align:right;">
        <el-button type="danger" @click="dialogVisibleGoods=false">取消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    express(scope) {
      ...
      express({orderId:scope.row.id}).then((res) => {
        ...
        console.log(res.data.data.expressInfo)
      });
    },
    async detailOrder(scope) {
      this.showGoods = [];
      this.dialogVisibleGoods = true;
      this.order = deepClone(scope.row.id);
