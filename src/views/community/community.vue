<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      >
      <el-button
        v-permission="['POST /admin/community/create']"
        class="filter-item"
        type="primary"
        icon="el-icon-edit"
        @click="handleCreate"
    ...
    >
      ...
      <el-table-column align="center" label="省市区">
        <template slot-scope="scope">
          {{ scope.row.province == scope.row.city ? "" : scope.row.province }}
      ...
      <el-table-column align="center" label="开通配送" prop="contract">
        <template slot-scope="scope">
          <el-tag :type="scope.row.contract ? 'success' : 'error'">{{
            scope.row.contract ? "已开通" : "未开通"
          }}</el-tag>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            v-permission="['POST /admin/community/update']"
            type="primary"
            size="mini"
          ...
          >
          <el-button
            v-permission="['POST /admin/community/delete']"
            type="danger"
            size="mini"
...
export default {
  ...
  methods: {
    ...
    addDeliveryPoints() {
      ...
      this.dataForm.deliveryPoints.push({
        contactName: "",
        contactMobile: "",
        addressDetail: ""
      });
    ...
    getProvince() {
      listSubRegion(this.regionQuery)
        .then(response => {
    ...
    handleUpdate(row) {
      ...
      if (row.weixinWorkGroupId) {
        this.wxworkAdmin = {
          id: row.weixinWorkGroupId,
          groupName: row.weixinWorkGroupName
        };
        this.wxworkList = [
          { id: row.weixinWorkGroupId, groupName: row.weixinWorkGroupName }
        ];
      } else {
        this.wxworkAdmin = "";
        this.wxworkList = [];
