<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button v-permission="['GET /admin/coupon/list']" :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-download" @click="handleDownload">导出</el-button>
    </div>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="商品限制范围">
          <el-radio-group v-model="dataForm.goodsType" @change="goodsTypeChange">
        ...
        <el-form-item v-show="dataForm.goodsType === 1">
          <el-select v-model="categoryId" multiple value-key="id" class="filter-item" style="width:100%;" placeholder="请选择" @change="categoryChange">
            <el-option
...
export default {
  ...
  methods: {
    getCatL1() {
      listCatL1().then(response => {
        ...
        for (const idx in response.data.data.list) {
          this.categoryList.push({ id: response.data.data.list[idx].value, name: response.data.data.list[idx].label })
        }
      })
    },
    categoryChange(e) {
      ...
      for (const idx in this.categoryList) {
        for (const idx1 in e) {
          if (e[idx1].id === this.categoryList[idx].id) {
            list.push(this.categoryList[idx])
          }
        }
      }
      this.categoryId = list
      const text = this.categoryId.map(function(v) {
        return v.name
      })
      this.dataForm.desc = '限' + text.join('、') + '用'
    },
    goodsTypeChange() {
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          if (this.dataForm.goodsType === 1) {
            ...
            this.dataForm.goodsValue = this.categoryId
          }
          if (this.dataForm.goodsType === 2) {
            ...
            const goodsValue = []
            for (const idx in this.goodsId) {
              goodsValue.push({ id: this.goodsId[idx].id, name: this.goodsId[idx].name })
            }
            this.dataForm.goodsValue = goodsValue
          }
    ...
    handleUpdate(row) {
      ...
      if (this.dataForm.goodsType === 1) {
        this.categoryId = row.goodsValue
      }
      if (this.dataForm.goodsType === 2) {
        this.goodsId = row.goodsValue
    ...
    handleDelete(row) {
      this.$confirm('确定要删除当前优惠券吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteCoupon({ id: row.id })
          .then(response => {
            this.$notify.success({
              title: '成功',
              message: '删除优惠券成功'
