<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" min-width="120" label="退款订单号" prop="orderSn">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="showOrderDetail(scope.row)">{{scope.row.orderSn}}</el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" label="订单状态" min-width="80" prop="orderStatus">
        <template slot-scope="scope">
          <span v-if="scope.row.orderStatus == 201">已支付</span>
          <span v-if="scope.row.orderStatus != 201">已发货</span>
        </template>
      </el-table-column>
...
//          this.detailAfterSale.push(this.afterSaleVo);
      ...
      handleRefund(row) {
        ...
        this.refundForm.actualRefundPrice = row.actualRefundPrice;
        this.refundForm.applyRefundPrice = row.actualRefundPrice;
        ...
        this.$refs['refundForm'].validate((valid) => {
          if (valid) {
            if (this.refundForm.actualRefundPrice > this.refundForm.applyRefundPrice) {
              this.$message.error('退款金额不能大于实际退款金额！')
              return false
            }
            if (this.refundForm.actualRefundPrice <= 0) {
