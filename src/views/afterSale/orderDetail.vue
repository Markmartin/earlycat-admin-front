          ...
          <el-col :span="24">
            <el-form-item label="商品信息">
              <el-form-item align="right">
                <el-button type="primary" v-if="orderDetail.order.orderStatus ===201" @click="handleAllRefund()">整单退款
                </el-button>
              </el-form-item>
              <el-table :data="orderDetail.orderGoods" border fit highlight-current-row
                ...
                <el-table-column align="center" label="操作" width="100" class-name="small-padding fixed-width">
                  <template slot-scope="scope">
                    ...
                    <el-button
                      v-if="(orderDetail.order.orderStatus !=201 && scope.row.refundNumber < scope.row.number) &&(scope.row.acStatus === 0||scope.row.acStatus === 1||scope.row.acStatus === 2)"
                      v-permission="['POST /admin/afterSale/refund']" type="warning " size="mini" round
                      @click="handleRefund(scope.row)">退款
                    </el-button>
                  </template>
              ...
              <el-container>
                ...
                <el-main width="300px">
                  ...
                  <el-form-item label="退款数量" prop="number">
                    <el-input-number v-model="item.number" :min="item.minNum" :max="item.maxNum"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleAllRefund() {
        ...
        for (var i = 0; i < orderGoodsList.length; i++) {
          if (orderGoodsList[i].acStatus != 98 && orderGoodsList[i].acStatus != 99) {
            ...
            this.refundForm.maxNum =  this.refundGoods.number;
            this.refundForm.refundGoodsList.push(this.refundGoods);
            const afterSaleItemVo = {
      ...
      handleRefund(row) {
        this.refundForm.maxNum = row.number;
        this.refundGoods = {};
        this.refundGoods = {
          orderId: row.orderId,
          goodsId: row.goodsId,
        ...
        getOrderGoodsRefundPrice(this.orderGoodsPrice).then(response => {
          ...
          const afterSaleItemVo = {
            ...
            number: row.number,
            price: this.refundForm.refundMoney,
          };
          this.afterSaleItemVos.push(afterSaleItemVo);
        }).catch(response => {
          this.$notify.error({
      ...
      confirmRefund() {
        const afterSaleVo = {
          userId: this.orderDetail.order.userId,
          orderId: this.orderDetail.order.id,
          type: 1,
          isAll: false,
          reason: this.refundForm.reason,
          url: this.url,
          detail: this.refundForm.detail,
          refundPrice: this.refundForm.refundMoney,
          orderStatus: this.orderDetail.order.orderStatus,
          afterSaleItemVos: this.afterSaleItemVos
        };
        applyAfterSale(afterSaleVo).then(response => {
          this.refundForm = {};
