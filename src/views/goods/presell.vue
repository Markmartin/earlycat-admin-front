<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-select v-model="listQuery.presellVo.acStatus" clearable style="width: 200px" class="filter-item" placeholder="请选择限时销售状态">
        <el-option v-for="type in isStatusOption" :key="type.value" :label="type.label" :value="type.value"/>
      </el-select>
      <el-select v-model="listQuery.presellVo.type" clearable style="width: 200px" class="filter-item" placeholder="请选择限时销售类型">
        <el-option v-for="type in typeOption" :key="type.value" :label="type.label" :value="type.value"/>
      </el-select>
      <el-input v-model="listQuery.presellVo.title" clearable class="filter-item" style="width: 200px;" placeholder="请输入限时销售标题"/>
      <el-button v-permission="['GET /admin/presell/list']" class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">查找
      </el-button>
      <el-button v-permission="['POST /admin/presell/create']" class="filter-item" type="primary" icon="el-icon-edit" @click="handleCreate">添加
      </el-button>
    </div>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px"
        ...
        <el-form-item label="限时销售类型" prop="acStatus">
          <el-select v-model="dataForm.type" placeholder="请选择限时销售类型">
            <el-option v-for="type in typeOption" :key="type.value" :label="type.label" :value="type.value"/>
          </el-select>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleItemGoods(row) {
        this.$router.push({ path: '/goods/presellItems', query: { id: row.id, type: row.type } })
      }
    }
  }
</script>
